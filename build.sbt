...
lazy val `http4s-consul-middleware` = crossProject(JSPlatform, JVMPlatform)
  ...
  .settings(
    ...
    libraryDependencies ++= {
      Seq(
        ...
        "org.tpolecat" %%% "natchez-noop" % "0.3.9",
  ...
  .jvmSettings(
    libraryDependencies ++= Seq(
      "org.tpolecat" %%% "natchez-jaeger" % "0.3.9" % Test,
    )
  )
  .jsSettings(
    libraryDependencies ++= Seq(
      "org.tpolecat" %%% "natchez-noop" % "0.3.9" % Test,
...
lazy val `consul-discoverable-smithy-spec` = project
  ...
  .settings(
    ...
    libraryDependencies += "software.amazon.smithy" % "smithy-model" % "1.68.0",
