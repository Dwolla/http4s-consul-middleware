...
lazy val http4sVersion = "0.23.35"
...
lazy val `http4s-consul-middleware` = crossProject(JSPlatform, JVMPlatform)
  ...
  .jvmSettings(
    libraryDependencies ++= Seq(
      "org.tpolecat" %%% "natchez-jaeger" % "0.3.10" % Test,
    )
  )
  .jsSettings(
    libraryDependencies ++= Seq(
      "org.tpolecat" %%% "natchez-noop" % "0.3.10" % Test,
...
lazy val `consul-discoverable-smithy-spec` = project
  ...
  .settings(
    ...
    libraryDependencies += "software.amazon.smithy" % "smithy-model" % "1.72.0",
